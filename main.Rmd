...
print.draws_summary <- function(x, ...) print(as.data.frame(x))
```
...
simple_poisson_dgp(
  ...
  alpha_sd = 1,
  beta_mean = -0.25,  # log(a little less than one)
  beta_sd = 0.5
)
```
...
This is still not great, but more reasonable than the previous one. In general,
it's difficult for this example to find the right parameter values for a Poisson
that make the predictions highly plausible. For example, when checking the mean
of the prior predictions some outlandishly high maximums are too influential
(the quartiles are more reasonable). For now we'll still use this because it's the
simplest count model. We will get more realistic as we develop a better model
later.
